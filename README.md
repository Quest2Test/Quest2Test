# QA Portfolio - [Quest2Test]

**Technical Support Engineer → QA Engineer** | UK/FR | 

[![MIT License](https://img.shields.io/badge/license-MIT-blue.svg)](LICENSE)
[![Profile Views](https://komarev.com/ghpvc/?username=Quest2Teste&color=green)](https://github.com/Quest2Test/Quest2Test)

> [!TIP]
> 💡 Choose your language / Choisissez votre langue:
>
> - 🇬🇧 [English](./README.md)
> - 🇫🇷 [Français](./README.fr.md)

## 📋 About Me
IT Support professional with hands-on experience across applications support, ISO-audited processes, and structured troubleshooting, now building on that foundation with QA testing skills. Comfortable owning a problem end-to-end: diagnosing root cause, working with vendors and stakeholders to fix it, and documenting it so the next person doesn't have to start from scratch.

Strengths: troubleshooting, structured investigation, detail-oriented issue resolution, hands-on testing across devices. Outside of work: video games, photography, and building on both my IT and QA skill sets.

Games I've played: [Backloggd](https://backloggd.com/u/Quest2Test/)

## Skills & Tools
 
### QA & Testing
| Tool / Skill | Level |
|---|---|
| TestRail | Certified — test case management, test runs, reporting |
| Qase | Familiar — test suite creation, defect logging |
| Jira | Familiar — issue tracking, sprint boards |
| pytest | In progress — automated test scripting in Python |
| Postman | In progress — API testing, collections, assertions |
| Bug reporting & escalation | Familiar — structured defect documentation |
| Test case documentation | Familiar — step-by-step, expected vs actual |
 
### Technical
| Skill | Level |
|---|---|
| Windows 10/11 | Experienced |
| SQL | Intermediate — database queries, log analysis |
| Python | Basic — scripting, automation, pytest |
| HTML & CSS | Basic |
| Remote Desktop / RDP | Experienced |
 
### Workflow & Collaboration
| Tool | Experience |
|---|---|
| Asana | Certified |
| Discord | Personal Use |
| Notion | Certified & Personal Use |
| FreshDesk (ITSM) | Experienced — Ticketing, SLA management, Asset Management |
| Microsoft Teams | Experienced |
| Slack | Personal learning |
| Unity | Personal learning |

## 📚 Certifications & Learning
  - 🏅 CompTIA A+
  - 🏅 TestRail Fundamentals
  - 🏅 Asana Foundation Course
  - 🏅 Notion Essentials Badge
  - 🏅 ISO 9001:2015 Internal Auditor Training
  - 🏅 ArcGIS Pro Foundation Course
  - 🏅 Unity Essentials
  - 🏅 Postman API Beginner Learning Path 
<!--
  Hide until ready
  - ISTQB CTFL 4.0: Studying Fundamentals
  - uTest Academy: Completed 
  - Google Agile Essentials: Completed [badge]
  - Qase Academy: Completed [badge]
  - Guru99 Mobile Testing Project: Completed [badge]
-->

⭐ Star this repo to support!


*Last updated: 2026 · [github.com/Quest2Test](https://github.com/Quest2Test)*
